<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:51:"./application/admin/view2/promotion/flash_sale.html";i:1491382650;s:44:"./application/admin/view2/public/layout.html";i:1491382650;}*/ ?>
<!doctype html>
<html>
<head>
...
<div class="page">
	...
	<div class="flexigrid">
		<div class="mDiv">
			...
			<form class="navbar-form form-inline" id="search-form" action="<?php echo U('Promotion/flash_sale'); ?>" method="post" onsubmit="return check_form();">
				...
				<div class="sDiv">
					...
					<div class="sDiv2">
						<select name="status" class="select">
							...
							<?php if(is_array($state) || $state instanceof \think\Collection): if( count($state)==0 ) : echo "" ;else: foreach($state as $key=>$st): ?>
				...
				<table>
					...
					<?php if(is_array($prom_list) || $prom_list instanceof \think\Collection): if( count($prom_list)==0 ) : echo "" ;else: foreach($prom_list as $k=>$vo): ?>
						<tr>
							...
							<td align="left" class="">
								<div style="text-align: center; width: 80px;"><?php if($vo[end_time] < time()): ?>已结束<?php else: ?><?php echo $state[$vo[status]]; endif; ?></div>
							</td>
							...
							<td align="left" class="handle">
								<div style="text-align: left; width: 170px; max-width:170px;">
									<?php if($vo['status'] == 0): ?>
										<a class="btn blue" onclick="changeStatus(1,'<?php echo $vo['id']; ?>','flash_sale')"><i class="fa fa-check"></i>通过</a>
										<a class="btn red" onclick="changeStatus(2,'<?php echo $vo['id']; ?>','flash_sale')"><i class="fa fa-ban"></i>拒绝</a>
									<?php endif; if($vo['status'] == 1): ?>
										<a class="btn red" onclick="changeStatus(3,'<?php echo $vo['id']; ?>','flash_sale')"><i class="fa fa-close"></i>取消</a>
									<?php endif; ?>
