...
class System extends Base{
	...
	{          
		/*配置列表*/
		$group_list = array('shop_info'=>'网站信息','basic'=>'基本设置','sms'=>'短信设置','shopping'=>'购物流程设置','smtp'=>'邮件设置','water'=>'水印设置', 'settled'=>'入驻设置','flow'=>'流水设置','envelopes'=>'红包设置');
		$this->assign('group_list',$group_list);
		$inc_type =  I('get.inc_type','shop_info');
		$this->assign('inc_type',$inc_type);
		if ($inc_type == 'settled'){
		    $settled = M('business_fee_bill')->where('id',1)->select();
		    $this->assign('config',$settled[0]);
        } else if($inc_type == 'flow') {
		    $flow = M('merchant_flow_bill')->where('id',1)->select();
		    $this->assign('config',$flow[0]);
        } else if ($inc_type == 'envelopes'){
		    $envelopes = M('business_ad_bill')->where('id',1)->select();
		    $this->assign('config',$envelopes[0]);
        } else {
     ...
     */
     ...
     public function set_settled(){
         $data = I('post.');
         $data['inc_type'] = 'settled';
         if($data['province'] == ''){
             $this->error("省代理入驻提成比例不能为空");
         }
         if ($data['city'] == ''){
             $this->error("市代理入驻提成比例不能为空");
         }
         if ($data['district'] == ''){
             $this->error("区/县代理入驻提成比例不能为空");
         }
         if ($data['referrer'] == ''){
             $this->error("推荐人提成比例不能为空");
         }
         if ($data['qz_direct'] == ''){
             $this->error("全职铁军入驻提成比例不能为空");
         }
         if ($data['jz_iron_army'] == ''){
             $this->error('兼职铁军入驻提成比例不能为空');
         }
         if ($data['qz_indirect'] == ''){
             $this->error("全职铁军间接入驻提成比例不能为空");
         }
         $res = M('business_fee_bill')->where('id',1)->save($data);
         if ($res){
             $this->success('操作成功',U('System/index',array('inc_type'=>$data['inc_type'])));
         } else {
             $this->error("操作失败");
         }
     }
     ...
     public function set_flow()
     {
         ...
         $data['inc_type'] = 'flow';
         if ($data['province'] == ''){
             $this->error("省代理商家流水提成比例不能为空");
         }
         if ($data['city'] == ''){
             $this->error("市代理商家流水提成比例不能为空");
         }
         if ($data['district'] == ''){
             $this->error("区/县代理商家流水提成比例不能为空");
         }
         if ($data['referrer'] == ''){
             $this->error("推荐商户流水提成比例不能为空");
         }
         if ($data['referrer_time'] == ''){
             $this->error("推荐享受商户流水提成时长不能为空");
         }
         if ($data['qz_direct'] == ''){
             $this->error("全职铁军商户流水提成比例不能为空");
         }
         if ($data['qz_indirect'] == ''){
             $this->error("全职铁军间接商户流水提成比例不能为空");
         }
         if ($data['sg_fans_bill'] == ''){
             $this->error("商户粉丝消费流水提成比例不能为空");
         }
         if ($data['jz_fans_bill'] == ''){
             $this->error("兼职铁军商户粉丝消费流水提成比例不能为空");
         }
         if ($data['qz_fans_direct'] == ''){
             $this->error("全职铁军商户粉丝消费流水提成比例不能为空");
         }
         if ($data['qz_fans_indirect'] == ''){
             $this->error("全职铁军间接商户粉丝消费流水提成比例不能为空");
         }
         $res = M('merchant_flow_bill')->where('id',1)->save($data);
         if ($res){
             $this->success('操作成功',U('System/index',array('inc_type'=>$data['inc_type'])));
         } else {
             $this->error("操作失败");
      ...
      * 设置广告红包分红比例
      */
     public function set_envelopes()
     {
         $data = I('post.');
         $data['inc_type'] = 'envelopes';
         if ($data['province'] == ''){
             $this->error("省代理红包分红比例不能为空");
         }
         if ($data['city'] == ''){
             $this->error("市代理红包分红比例不能为空");
         }
         if ($data['district'] == ''){
             $this->error("区/县代理红包分红比例不能为空");
         }
         if ($data['platform_fee'] == ''){
             $this->error("平台服务费比例");
         }
         if ($data['share_red_packet'] == ''){
             $this->error("分享红包奖励比例");
         }
         if ($data['share_real_buy'] == ''){
             $this->error("分享朋友圈获得商品返利比例");
         }
         if ($data['red_packet_drainage'] == ''){
             $this->error('红包引流率不能为空');
         }
         $res = M('business_ad_bill')->where('id',1)->save($data);
         if ($res){
             $this->success('操作成功',U('System/index',array('inc_type'=>$data['inc_type'])));
         } else {
             $this->error("操作失败");
     ...
     public function testBonus()
     {
        ...
        $ordersn = 'IA20191016150632';
//        $district = '渝中区';
//        $district = json_encode($district);
//        var_dump($district);die;

        $res = $model->merchant_entry($id,$money,$area,$ordersn);
        $this->fetch($res);
     }
}