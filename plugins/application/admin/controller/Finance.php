...
class Finance extends Base {
    ...
    public function get_user_transaction()
    {
        $create_time = I('create_time');
        $create_time = str_replace("+"," ",$create_time);
        $create_time2 = $create_time  ? $create_time  : date('Y-m-d',strtotime('-1 month')).' - '.date('Y-m-d',strtotime('+1 month'));
        $create_time3 = explode(' - ',$create_time2);
        $this->assign('start_time', $create_time3[0]);
        $this->assign('end_time', $create_time3[1]);
        $where['time'] =  array(array('gt', strtotime(strtotime($create_time3[0])), array('lt', strtotime($create_time3[1]))));
        $keyword = empty($keyword)?I('keyword'):$keyword;
        $keyword && $where['nickname|ordersn'] = array('like','%'.$keyword.'%');
        $count = Db::name('record_transaction')->where($where)->count();
        $Page  = new Page($count,16);
        $list = Db::name('record_transaction')->where($where)->order("`id` desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        $show  = $Page->show();
        $this->assign('pager',$Page);
        $this->assign('show',$show);
        $this->assign('list',$list);
    ...
    public function get_user_bonus()
    {
        ...
        $where['time'] =  array(array('gt', strtotime(strtotime($create_time3[0])), array('lt', strtotime($create_time3[1]))));
        $keyword = empty($keyword)?I('keyword'):$keyword;
        $keyword && $where['nickname|tid'] = array('like','%'.$keyword.'%');
        $count = Db::name('record_bonus')->where($where)->count();
        $Page  = new Page($count,16);
        $list = Db::name('record_bonus')->where($where)->order("`id` desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        $show  = $Page->show();
        $this->assign('pager',$Page);
        $this->assign('show',$show);
        $this->assign('list',$list);
