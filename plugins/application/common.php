...
function update_pay_status($order_sn, $transaction_id = '', $tid= 0 )
{
    if (stripos($order_sn, 'recharge') !== false) {
        ...
        // 修改支付状态  已支付
        M('order')->where("order_sn", $order_sn)->update(array('pay_status' => 1, 'pay_time' => time(), 'transaction_id' => $transaction_id));
        // 减少对应商品的库存
        minus_stock($order);
        //绑定粉丝商户
        $bindStore = M('users')->where('user_id',$order['user_id'])->value('store_id');
        if ($bindStore == ''){
            M('users')->where('user_id',$order['user_id'])->save(array('store_id'=>$order['store_id']));
        }
        if ( $tid != 0 ) {
            fans_water( $order['store_id'], $order['goods_price'], 2, 2, $order['user_id'], $order['order_sn'], $order['pay_time'], $tid );
        } else {
            fans_water( $order['store_id'], $order['goods_price'], 2, 2, $order['user_id'], $order['order_sn'], $order['pay_time'] );
        }
...
function merchant_water($user_id = null, $money = null, $ordertype = null, $type = null, $paytime = null, $ordersn = '', $share_id = null)
{
    ...
    //如果购买时有分享人
    if ($share_id != '') {
        file_put_contents('wxshopinfo',$share_id);
        ...
        if ($rico) {
            add_order($share_id, $allMoney, $ordertype, $type, '分享购买分红', $moneydata, $ordersn);
        }
    }
    if ($referrer) {
        if ($referrer['level'] != 3) {//一般推荐人
            if ((time() - $paytime) <= 60 * 60 * 24 * $rate['referrer_time']) {//判断是否超过可拿奖励时间
                ...
                if ($res) {
                    add_order($referrer['user_id'], $allMoney, $ordertype, $type, '推荐人商户流水分润', $moneydata, $ordersn);
                }
            }
            $qz = getParents($referrer['referrer_id']);//全职铁军间接分红
            if ($qz) {
...
//        merchant_apply($money,$platform_fee,$ordertype,$type ,$area,$ordersn );
    ...
    $re = M('users')->where('user_id', $owner['user_id'])->save(array('frozen_funds' => $owner['frozen_funds'] + $owner_amount));
    if ($re) {
        add_order($owner['user_id'], $allMoney, $ordertype, $type, '商户粉丝消费分润', $moneydata, $ordersn);
    }
    //粉丝商户推荐人
    $referrer = M('users')->where('user_id', $owner['referrer_id'])->find();
    if ($referrer) {
        if ($referrer['level'] == 2) {//兼职铁军可拿商户粉丝消费流水
            ...
            if ($rico) {
                add_order($referrer['user_id'], $allMoney, $ordertype, $type, '兼职铁军商户粉丝消费分润', $moneydata, $ordersn);
            }
            $qz = getParents($referrer['referrer_id']);//全职铁军间接分红
            if ($qz) {
                ...
                $res = M('users')->where('user_id', $qz['user_id'])->save(array('frozen_funds' => $qz['frozen_funds'] + $qz_amount));
                if ($res) {
                    add_order($qz['user_id'], $allMoney, $ordertype, $type, '全职铁军间接商户粉丝消费分润', $moneydata, $ordersn);
                }
            }
        }
        if ($referrer['level'] == 3) {//全职铁军可拿商户粉丝消费流水
            ...
            $rico = M('users')->where('user_id', $referrer['user_id'])->save(array('frozen_funds' => $referrer['frozen_funds'] + $amount));
            if ($rico) {
                add_order($referrer['user_id'], $allMoney, $ordertype, $type, '全职铁军商户粉丝消费分润', $moneydata, $ordersn);
            }
        }
    }
}
