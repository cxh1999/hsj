...
{    
    ...
    /**
     * 商家入驻/订单支付 jsapi支付回调查询订单方法
     * 
     */
    function responseShop( $transaction_id )
    {                        
        require_once("example/notify.php");  
        $notify = new PayNotifyCallBack();
        $response = $notify->Queryorder($transaction_id);
        return $response;      
    }
    ...
    商家入驻单独jsapi支付调取
     */
    function getshopJSAPI($order){
        require_once("example/WxPay.JsApiPay.php");
        $go_url = U('Mobile/User/shopinfo_notice');
        $back_url = U('Mobile/User/shopadopt_pay');
        //①、获取用户openid
        $tools = new JsApiPay();
        //$openId = $tools->GetOpenid();
        $openId = $_SESSION['openid'];
        $ordersn = $order['ordersn'].time();
        $uid = $_SESSION['shopinfo']['user_id'];
        $uname = $_SESSION['shopinfo']['user_name'];
        $pwd = $_SESSION['shopinfo']['password'];
        //②、统一下单
        $input = new WxPayUnifiedOrder();
        $input->SetBody("商家入驻：".$ordersn);
        $input->SetAttach("weixin");
        $input->SetOut_trade_no($ordersn);
        $input->SetTotal_fee($order['sg_price']*100);
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag("sku_shop_info");
        file_put_contents('1',$_SESSION['shopinfo']['seller_name']);
        $input->SetNotify_url(SITE_URL.'/index.php/Home/Payment/shopNotify/pay_code/weixin/ordersn/'.$ordersn.'/uid/'.$uid .'/uname/'.$uname.'/pwd/'.$pwd);
...
EOF;
    ...
    function getJSAPI( $orders ){
    	require_once("example/WxPay.JsApiPay.php");
    	if(stripos($orders['order_sn'],'recharge') !== false){
    		$go_url = U('Mobile/User/order_detail',array('type'=>'recharge'));
    		$back_url = U('Mobile/User/recharge',array('order_id'=>$orders['order_id']));
    	}else{
    		$go_url = U('Mobile/User/order_detail',array('id'=>$orders['order_id']));
    		$back_url = U('Mobile/Cart/cart4',array('order_id'=>$orders['order_id']));
    	}
        //①、获取用户openid
        $tools = new JsApiPay();
        $openId = $_SESSION['openid'];
        ...
        //②、统一下单
        $input = new WxPayUnifiedOrder();
        $input->SetBody("支付订单：".$orders['order_sn']);
        $input->SetAttach("weixin_".$orders['tid']);
        $input->SetOut_trade_no($orders['order_sn'].time());
        $input->SetTotal_fee($orders['order_amount']*100);
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag("shop_pay");
        $input->SetNotify_url(SITE_URL.'/index.php/Home/Payment/notifyUrl/pay_code/weixin/tid/'.$tid);
        $input->SetTrade_type("JSAPI");
        $input->SetOpenid($openId);
        $order2 = WxPayApi::unifiedOrder($input);
        $jsApiParameters = $tools->GetJsApiParameters($order2);
	...
	function jsApiCall()
	{
		WeixinJSBridge.invoke(
			...
			function(res){
				//WeixinJSBridge.log(res.err_msg);
				 if(res.err_msg == "get_brand_wcpay_request:ok") {
				    location.href='http://skhcenter.baoliy168.com/Mobile/User/order_list/type/WAITSEND.html';
				 }else{
				 	alert(res.err_code+res.err_desc+res.err_msg);
				    location.href='$back_url';
				 }
