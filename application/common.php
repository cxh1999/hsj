 ...
 * $tid 分享老师id
 */
function update_pay_status($order_sn, $transaction_id = '', $tid= 0 )
{
    ...
    if (stripos($order_sn, 'recharge') !== false) {
        ...
        if ($order_list) {
            ...
            update_pay_status($val['order_sn'], $transaction_id, $tid );
            return;
        }
        // 如果这笔订单已经处理过了
        ...
        // 修改支付状态  已支付
        M('order')->where("order_sn", $order_sn)->update(array('pay_status' => 1, 'pay_time' => time(), 'transaction_id' => $transaction_id ,'pay_code' => $payCode));
        // 减少对应商品的库存
        minus_stock($order);
        //商户流水分红
        $store_time = M('store')->where('store_id',$order['store_id'])->value('store_time');
        if ( $tid != 0 ) {
            fans_water( $order['store_id'], $order['goods_price'], 2, 2, $order['user_id'], $order['order_sn'], $store_time, $tid );
        } else {
            fans_water( $order['store_id'], $order['goods_price'], 2, 2, $order['user_id'], $order['order_sn'], $store_time );
        }
 ...
 */
//图片上传
function uploadFile($file, $bucket = 0)
{
    $qiniu = new \Qiniu\Auth(C('qiniu.accessKey'), C('qiniu.secretKey'));
    // 获取上传凭证token
    $token = $qiniu->uploadToken(C('qiniu.bucket')[$bucket]);
    // 上传文件的本地路径
    $filePath = $file["tmp_name"];
    // 获取文件后缀名
    $extension = explode(".", $file["name"]);
    // 验证图片格式是否正确
    $validateFormat = validateFormat($file);
    if ($validateFormat["code"] != 1) {
        return $validateFormat;
    }
    // 上传到七牛后保存的文件名
    $key = md5(uniqid(microtime(true), true)) . "." . end($extension);
 ...
 * @return array
 */
function findFile($key, $bucket = 0)
{
    $auth = new \Qiniu\Auth(C('qiniu.accessKey'), C('qiniu.secretKey'));
    $bucketManager = new \Qiniu\Storage\BucketManager($auth);
    $res = $bucketManager->stat(C('qiniu.bucket')[$bucket], $key);
 ...
 * @return array
 */
function unFile($key, $bucket = 0)
{
    $auth = new \Qiniu\Auth(C('qiniu.accessKey'), C('qiniu.secretKey'));
    $bucketManager = new \Qiniu\Storage\BucketManager($auth);
    $result = findFile($key, $bucket);
    if ($result["code"] == 1) {
        $res = $bucketManager->delete(C('qiniu.bucket')[$bucket], $key);
        if ($res == NULL) {
 ...
 */
function merchant_entry($user_id = null, $money = null, $ordertype = null, $type = null, $ordersn = '')
{
    //入驻提成比例
    ...
    //省市区代理分红
//        merchant_apply($money,$platform_fee,$ordertype,$type ,$area,$ordersn );
    switch ($referrer['level']) {
        case '1'://普通推荐人
            ...
            if ($res) {
                add_order($referrer['user_id'], $allmoney, $ordertype, $type, '商户推荐奖', $moneydata, $ordersn);
            }
            $qz = getParents($referrer['referrer_id']);//全职铁军间接分红
            if ($qz) {
                ...
                if ($re) {
                    add_order($qz['user_id'], $allmoney, $ordertype, $type, '全职铁军间接商户推荐奖', $moneydata, $ordersn);
                }
            }
            break;
        case '2'://兼职铁军
            ...
            $res = M('users')->where('user_id', $referrer['user_id'])->save(array('user_money' => $referrer['user_money'] + $amount));
            if ($res) {
                add_order($referrer['user_id'], $allmoney, $ordertype, $type, '兼职铁军商户推荐奖', $moneydata, $ordersn);
            }
            $qz = getParents($referrer['referrer_id']);//全职铁军间接分红
            if ($qz) {
        ...
        case '3'://全职铁军
            ...
            $res = M('users')->where('user_id', $referrer['user_id'])->save(array('user_money' => $referrer['user_money'] + $amount));
            if ($res) {
                add_order($referrer['user_id'], $allmoney, $ordertype, $type, '全职铁军商户推荐奖', $moneydata, $ordersn);
            }
            break;
    }
 ...
 * @param string $ordersn 交易订单号
 */
function merchant_apply($money = null, $platform_fee = null, $ordertype = null, $type = null, $area = array(), $ordersn = '')
{
    if ($type == 1 || $type == 4) {
    ...
    if ($userProvince) {
        ...
        $p = M('users')->where('user_id', $userProvince['uid'])->save(array($field => $province[$field] + $provinceMoney));
        if ($p) {
            if ($ordertype == 1 ) {
                $explain = '省代理商户入驻分润';
            } else if ($ordertype == 2) {
            ...
            add_order($province['user_id'], $allMoney, $ordertype, $type, $explain, $moneydata, $ordersn);
        }
    }
    //市
    ...
    if ($userCity) {
        ...
        if ($member) {
            //更新会员数据
            $userAttributes = M('users')->where('user_id', $user_id)->save(array($field => $member[$field] + $cityMoney));
            if ($userAttributes) {
                if ($ordertype == 1 ) {
                    $explain = '市代理商户入驻分润';
                } else if ($ordertype == 2) {
                ...
                //添加记录
                add_order($member['user_id'], $allMoney, $ordertype, $type, $explain, $moneydata, $ordersn);
            }
        }
    }
    //区
    ...
    if ($userDistrict) {
        ...
        if ($member) {
            ...
            //更新会员数据
            $userAttributes = M('users')->where(['user_id' => $member['user_id']])->save(array($field => $member[$field]));
            if ($userAttributes) {
                if ($ordertype == 1 ) {
                    $explain = '区/县代理商户入驻分润';
                } else if ($ordertype == 2) {
 ...
 * @param null $ordertype 交易类型
 * @param null $type 订单类型
 * @param null $paytime 店铺入驻支付时间
 * @param string $ordersn 交易订单号
 * @param null $share_id 分享人id(如果有)
 */
function merchant_water($user_id = null, $money = null, $ordertype = null, $type = null, $paytime = null, $ordersn = '', $share_id = null)
{
    //商户流水提成比例
    $rate = M('merchant_flow_bill')->where('id', 1)->find();
    //商家分享返利
    $share = M('store')->where('user_id', $user_id)->value('share_rebates');
    //平台费
    $platform_fee = M('config')->where('name', 'platform_fee')->value('value');
    $platform_fee = (int)$platform_fee;
    //商户信息
    ...
    if ($share_id != '') {
        ...
        $allMoney = $money * $share / 100;
        $prize = $money * $share / 100 * (100 - $platform_fee) / 100;
        $platform = $money * $share / 100 * $platform_fee / 100;
        $moneydata = ['money' => $prize, 'platform' => $platform];
        $rico = M('users')->where('user_id', $share_id)->save(array('frozen_funds' => $shareMan['frozen_funds'] + $prize));
        if ($rico) {
            add_order($share_id, $allMoney, $ordertype, $type, '分享购买返利', $moneydata, $ordersn);
        }
    }
    if ($referrer) {
        if ($referrer['level'] ==1) {//普通粉丝
            if ((time() - $paytime) <= 60 * 60 * 24 * $rate['referrer_time']) {//判断是否超过可拿奖励时间
                ...
                $moneydata = ['money' => $amount, 'platform' => $platform];
                $res = M('users')->where('user_id', $referrer['user_id'])->save(array('frozen_funds' => $referrer['frozen_funds'] + $amount));
                if ($res) {
                    add_order($referrer['user_id'], $allMoney, $ordertype, $type, '推荐商户流水分润', $moneydata, $ordersn);
                }
            }
            $qz = getParents($referrer['referrer_id']);//全职铁军间接分红
            if ($qz) {
                ...
                $moneydata = ['money' => $qz_amount, 'platform' => $platform];
                $res = M('users')->where('user_id', $qz['user_id'])->save(array('frozen_funds' => $qz['frozen_funds'] + $qz_amount));
                if ($res) {
                    add_order($qz['user_id'], $allMoney, $ordertype, $type, '全职铁军间接商户流水分润', $moneydata, $ordersn);
                }
            }
        } else if($referrer['level'] == 2){//兼职铁军
            $allMoney = $money * $store['service_fee'] / 100 * $rate['referrer'] / 100;
            $amount = $money * $store['service_fee'] / 100 * $rate['referrer'] / 100 * (100 - $platform_fee) / 100;
            $platform = $money * $store['service_fee'] / 100 * $rate['referrer'] / 100 * $platform_fee / 100;
            $moneydata = ['money' => $amount, 'platform' => $platform];
            $res = M('users')->where('user_id', $referrer['user_id'])->save(array('frozen_funds' => $referrer['frozen_funds'] + $amount));
            if ($res) {
                add_order($referrer['user_id'], $allMoney, $ordertype, $type, '兼职铁军商户流水分润', $moneydata, $ordersn);
            }
            $qz = getParents($referrer['referrer_id']);//全职铁军间接分红
            if ($qz) {
                $allMoney = $money * $store['service_fee'] / 100 * ($rate['qz_direct'] - $rate['referrer']) / 100;
            ...
            $res = M('users')->where('user_id', $referrer['user_id'])->save(array('frozen_funds' => $referrer['frozen_funds'] + $amount));
            if ($res) {
                add_order($referrer['user_id'], $allMoney, $ordertype, $type, '全职铁军商户流水分润', $moneydata, $ordersn);
            }
        }
    }
}
 ...
 * 商户粉丝消费分红规则
 */
function fans_water($store_id = null, $money = null, $ordertype = null, $type = null, $user_id = null, $ordersn = '', $paytime = null, $share_id = null)
{
    //商户流水提成比例
    $rate = M('merchant_flow_bill')->where('id', 1)->find();
    //平台费
    ...
    //        merchant_apply($money,$platform_fee,$ordertype,$type ,$area,$ordersn );
    //商户流水分润
    merchant_water($store['user_id'], $money, $ordertype, $type, $paytime, $ordersn, $share_id);
    //1客户老师
    $referrer = M('users')->where('user_id',$user['referrer_id'])->find();
    if ($referrer) {//推荐老师提成
        $allMoney = $money * $store['service_fee'] / 100 * $rate['fans_referrer'] / 100;
        $amount = $money * $store['service_fee'] / 100 * $rate['fans_referrer'] / 100 * (100 - $platform_fee) / 100;
        $platform = $money * $store['service_fee'] / 100 * $rate['fans_referrer'] / 100 * $platform_fee / 100;
        $moneydata = ['money' => $amount, 'platform' => $platform];
        $rico = M('users')->where('user_id', $referrer['user_id'])->save(array('frozen_funds' => $referrer['frozen_funds'] + $amount));
        if ($rico) {
            add_order($referrer['user_id'], $allMoney, $ordertype, $type, '推荐用户消费分润', $moneydata, $ordersn);
        }
        //2客户粉丝商户
        $storeUser = getStore($referrer['user_id']);
        if ($storeUser){
            //奖励
            $allMoney = $money * $store['service_fee'] / 100 * $rate['sg_fans_bill'] / 100;
            $amount = $money * $store['service_fee'] / 100 * $rate['sg_fans_bill'] / 100 * (100 - $platform_fee) / 100;
            $platform = $money * $store['service_fee'] / 100 * $rate['sg_fans_bill'] / 100 * $platform_fee / 100;
            $moneydata = ['money' => $amount, 'platform' => $platform];
            $rico = M('users')->where('user_id', $storeUser['user_id'])->save(array('frozen_funds' => $storeUser['frozen_funds'] + $amount));
            if ($rico) {
                add_order($storeUser['user_id'], $allMoney, $ordertype, $type, '商户粉丝消费分润', $moneydata, $ordersn);
            }
            //3兼职铁军或全职铁军
            $storeReferrer = M('users')->where('user_id',$storeUser['referrer_id'])->find();
            if ($storeReferrer){
                if ($storeReferrer['level'] == 2){//兼职铁军
                    //奖励
                    ...
                    $moneydata = ['money' => $amount, 'platform' => $platform];
                    $rico = M('users')->where('user_id', $storeReferrer['user_id'])->save(array('frozen_funds' => $storeReferrer['frozen_funds'] + $amount));
                    if ($rico) {
                        add_order($storeReferrer['user_id'], $allMoney, $ordertype, $type, '兼职铁军商户粉丝消费分润', $moneydata, $ordersn);
                    }
                    //4全职铁军
                    $fullIron = getFullIron($storeReferrer['referrer_id']);//全职铁军
                    if ($fullIron){
                        //全职铁军商户粉丝消费间接分润
                        $allMoney = $money * $store['service_fee'] / 100 * $rate['qz_fans_indirect'] / 100;
                        $amount = $money * $store['service_fee'] / 100 * $rate['qz_fans_indirect'] / 100 * (100 - $platform_fee) / 100;
                        $platform = $money * $store['service_fee'] / 100 * $rate['qz_fans_indirect'] / 100 * $platform_fee / 100;
                        $moneydata = ['money' => $amount, 'platform' => $platform];
                        $rico = M('users')->where('user_id', $fullIron['user_id'])->save(array('frozen_funds' => $fullIron['frozen_funds'] + $amount));
                        if ($rico) {
                            add_order($fullIron['user_id'], $allMoney, $ordertype, $type, '全职铁军间接商户粉丝消费分润', $moneydata, $ordersn);
                        }
                    }
                } elseif ($storeReferrer['level'] == 3){//全职铁军
                    //全职铁军商户粉丝消费分润
                    ...
                    $rico = M('users')->where('user_id', $storeReferrer['user_id'])->save(array('frozen_funds' => $storeReferrer['frozen_funds'] + $amount));
                    if ($rico) {
                        add_order($storeReferrer['user_id'], $allMoney, $ordertype, $type, '全职铁军商户粉丝消费分润', $moneydata, $ordersn);
                    }
                } else {
                    $fullIron = getFullIron($storeReferrer['referrer_id']);//全职铁军
                    if ($fullIron){
                        ...
                        $rico = M('users')->where('user_id', $fullIron['user_id'])->save(array('frozen_funds' => $fullIron['frozen_funds'] + $amount));
                        if ($rico) {
                            add_order($fullIron['user_id'], $allMoney, $ordertype, $type, '全职铁军商户粉丝消费分润', $moneydata, $ordersn);
                        }
                    }
                }
            }
 ...
 */
function add_order($user_id = null, $money = null, $ordertype = null, $type = null, $explain = '', $manageMoney = array(), $ordersn = '')
{
    $user = M('users')->where('user_id', $user_id)->find();
    if ($user['username'] != ''){
        $name = $user['username'];
    ...
    if ($type == 2 && $ordertype == 2){
        $levelinDatas['bonus_status'] = 0;
    }
    if ($type != 2 && $ordertype == 2){
        $levelinDatas['bonus_status'] = 1;
    }
...
//递归查询上级全职铁军
function getParents($referrer_id)
{
...
//递归查询商家上级铁军
function getIronMan($user_id)
{
    $user = M('users')->where('user_id',$user_id)->find();
    if ($user){
        if ($user['level'] == 2 || $user['lever'] == 3){
            return $user;
        } else {
            $user_id = $user['referrer_id'];
            return getIronMan($user_id);
        }
    } else {
        return false;
    }
 ...
 * @return int
 */
function getTeamCount($uid,$type=0) {
    if (empty($type)){
        $Teams = array();//最终结果
        $uids = M('users')->where('referrer_id',$uid)->column('user_id');
        $Teams = array_merge($Teams,$uids);
        $members = M('users')->select();
        do {
            $othermids = array();
            $state = false;
            foreach ( $uids as $valueone ) {
                foreach ($members as $key => $valuetwo) {
                    if($valuetwo['referrer_id'] == $valueone){
                        $Teams[] = $valuetwo['user_id'];//找到我的下级立即添加到最终结果中
                        $othermids[] = $valuetwo['user_id'];//将我的下级id保存起来用来下轮循环他的下级
                        array_splice($members,$key,0);//从数组中移除元素，并用新元素取代它
                        $state = true;
                    }
                }
            }
            $uids = $othermids;//foreach中找到的我的下级集合,用来下次循环
        } while ( $state == true );
        $teamNum = count($Teams);
    }else{
        $list = Db::query("SELECT user_id,username,referrer_id FROM (SELECT @ids AS _ids,(
                                SELECT
                                    @ids := GROUP_CONCAT(user_id)
                                FROM
                                    skh_users
                                WHERE
                                    FIND_IN_SET(referrer_id, @ids)
                            ) AS cids,
                            @l := @l + 1 AS LEVEL
                        FROM
                            skh_users,
                    ...
                    skh_users DATA
                WHERE
                    FIND_IN_SET(DATA.user_id, id._ids)");
        $teamNum=count($list);
    }
    return $teamNum;
}
 ...
 */
function location(){
    $app = new \EasyWeChat\Foundation\Application(C('options'));
 ...
 * @return mixed
 */
function superior($id,$yesId=false){
    $sql="SELECT DISTINCT T2.user_id,T2.nickname FROM (SELECT @r AS _user_id, ( SELECT
					@r := referrer_id
				FROM
					skh_users
				WHERE
...
JOIN skh_users T2 ON T1._user_id = T2.user_id
WHERE
	T2.user_id != ".$id."
ORDER BY
	T1.lvl ASC";
