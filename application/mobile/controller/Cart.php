...
class Cart extends MobileBase {
    ...
    public function cart2()
    {
        ...
        foreach ($cartList as $k=>$v){
            $cartList[$k]['original_img'] = M('goods')->where(['goods_id'=>$v['goods_id']])->value("original_img");
        }
    ...
    public function cart3(){
		...
		if($result['status'] < 0)	
			exit(json_encode($result));
        if ($result['result']['user_money'] > 0 && $result['result']['user_money'] != $result['result']['goods_price'] + $result['result']['shipping_price'] )
        {
            $this->error('暂不支持混合支付');
        }
                $car_price = array(
                ...
                {  
                    ...
                    $result = $this->cartLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$user_note,$tid); // 添加订单
                    exit(json_encode($result));
                }
