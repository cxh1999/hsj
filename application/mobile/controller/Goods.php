...
class Goods extends MobileBase {
    ...
    public function goodsInfo(){
        ...
        $teacher_id = I("get.u/d" , 0);
        $openId = I('openid',0);
        if ($openId != 0){
            $teacher_id = M('users')->where('openid',$openId)->value('user_id');
        }
//        session('referrer_id',$teacher_id);
        $_SESSION['tid'] = $teacher_id;
        ...
        if ( !empty( $teacher_id ) && is_numeric( $teacher_id ) ) {
            $teacher_find = M( 'users' )->where( "user_id" , $teacher_id )->find();
            if ( $teacher_find ) {
                $user_referr_find = M( 'users' )->where( "user_id" , $this->user_id )->find();
                if ( $user_referr_find["referrer_id"] == 0 ) {
                   if ( $teacher_find['referrer_id'] != $user_referr_find[ 'user_id' ] ) {
                        $referrData['referrer_id'] =  $teacher_id;
                       M('users')->where( array( 'user_id' => $this->user_id ) )->save( $referrData );
                   }
