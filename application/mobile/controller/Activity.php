...
class Activity extends MobileBase {
    ...
    */ 
    public function group(){
        ...
        $group_buy_info = M('GroupBuy')->where("goods_id = $goods_id and start_time <=" .time()." and end_time >= ".time())->find(); // 找出这个商品
    ...
    public function group_list()
    {
        ...
        $list = M('GroupBuy')->where(time()." >= start_time and ".time()." <= end_time ")->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function ajaxGroupListGetMore(){
        $p = I('p',1);
        $list = M('GroupBuy')->where(time()." >= start_time and ".time()." <= end_time ")->page($p,2)->select(); // 找出这个商品
        $this->assign('list', $list);
        return $this->fetch();
    }
     ...
     public function ajaxDiscountListGetMore(){
        $p = I('p',1);
