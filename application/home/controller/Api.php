...
class Api extends Controller
{
    ...
    public function get_cates_m()
    {
        $parent_id = I('get.parent_id/d', '0'); // 商品分类 父id
        empty($parent_id) && exit('');
        $list = M('goods_category')->where(array('parent_id' => $parent_id))->select();
        foreach ($list as $k => $v) {
            $html .= "<input type='button' cates='2' name='subcate[]' data-id='{$v['id']}' rel='{$v['commission']}' id='goodsLast{$v['id']}' value='{$v['name']}' onClick='last_type_check({$v['id']})'>";
        }
        exit($html);
    }
