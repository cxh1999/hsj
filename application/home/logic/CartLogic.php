    ...
    {                   
        ...
        foreach ($cartList as $k=>$val){
        	...
        	$cartList[$k]['store_count']  = getGoodNum($val['goods_id'],$val['spec_key']); // 最多可购买的库存数量        	
                ...
                $total_price += $cartList[$k]['goods_num'] * $val['member_goods_price'];
        }
...
function cart_freight2($shipping_code,$province,$city,$district,$weight,$store_id='0')
{
   ...
   return $freight;  
     ...
     */
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = array(),$car_price,$user_note,$tid)
    {
        ...
        {                           
                ...
                {                        
                    update_pay_status($order_sn, 1,$tid); // 这里刚刚下的订单必须从主库里面去查
                }                 
