...
class UsersLogic extends Model
{
    ...
    public function login($username,$password){
        ...
        if(!$user){
          ...
          //查询用户信息之后, 查询用户的分享人
            $userId = $user['user_id'];
            $userName = M("user")->where("user_id = {$userId}")->getField("referrer_id");
            $referrerName = M("user")->where("user_id = {$userName}")->getField("realname");
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        if(check_mobile($post['mobile']))
            return array('status'=>-1,'msg'=>'手机号码格式不对','result'=>'');
