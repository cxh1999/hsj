    ...
    public function deliveryHandle($data,$store_id){
        ...
        foreach ($orderGoods as $k=>$v){
            ...
            if($v['is_send'] == 0 && in_array($v['rec_id'],$selectgoods)){
                ...
                $res['delivery_id'] = $did;
                //确认发货后获得奖励
                $bonusList = M('record_transaction')->where(['ordersn' => $order['order_sn'],'type' => 2,'order_type' => 2])->select();
                if ($bonusList){
                    foreach($bonusList as  $value){
                        //改变奖励状态
                        M('record_transaction')->where(['id' => $value['id']])->save(array('bonus_status' => 1));
                        $user = M('users')->where('user_id',$value['user_id'])->find();
                        $user['user_money'] = $user['user_money'] + $value['money'] * (100 - $platform_fee)/100;
