...
class Admin extends Base
{
    ...
    public function forget_pwd()
    {
        ...
        switch ($type) {
            case 1:
                if (!$userName) {
                    return $this->ajaxReturn(['code' => 0, 'msg' => '商家账号不能为空']);
                }
                $seller = M('seller')->where(array('seller_name' => $userName))->find();
                if (!$seller) {
                    return $this->ajaxReturn(['code' => 0, 'msg' => '商家账号不存在']);
                }
                return $this->ajaxReturn(['code' => 1, 'msg' => '成功']);
                break;
            case 2:
                if (!$phone) {
                    return $this->ajaxReturn(['code' => 0, 'msg' => '商家手机号不能为空']);
                }
                $seller = M('seller')->where(array('seller_name' => $userName))->find();
                $store = M('store')->where(array('store_id' => $seller['store_id'], 'store_phone' => $phone))->value('store_phone');
                if (!$store) {
                    return $this->ajaxReturn(['code' => 0, 'msg' => '商家手机号不存在']);
                }
                return $this->ajaxReturn(['code' => 1, 'msg' => '成功']);
                break;
            case 3:
                if (!preg_match("/^1[0-9]{10}$/", $phone)) {
                    return $this->ajaxReturn(['code' => 0, 'msg' => '手机号不正确']);
                }
                if (session('seller_sms_' . $phone)['time'] >= time()) {
                    return $this->ajaxReturn(['code' => 0, 'msg' => '短信获取太快,请一分钟后再试']);
                }
                $rand = rand(100000, 999999);
                session('seller_sms_' . $phone, [
                    'rand' => $rand,
                    'time' => time() + 60
                ]);
                //时间
                $data = [
                    'sid' => C('yunSms.accountSid'),
                    'token' => C('yunSms.authToken'),
                    'appid' => C('yunSms.appId'),
                    'templateid' => 510459,
                    'param' => $rand . ',10',
                    'mobile' => $phone,
                ];
                ...
                $data = httpRequest(C('yunSms.url'), 'post', json_encode($data), $header);
                $data = json_decode($data, true);
                if ($data['code'] == "000000") {
                    return $this->ajaxReturn(['code' => 1, 'msg' => '短信获取成功']);
                }
                return $this->ajaxReturn(['code' => 0, 'msg' => '短信获取失败']);
                break;
            case 4:
                $post = I('post.');
                ...
                if (empty($post['password'])){
                    return $this->ajaxReturn(['code' => 0, 'msg' => '新密码不能为空']);
                }
                if (empty($post['code'])){
                    return $this->ajaxReturn(['code' => 0, 'msg' => '验证码不能为空']);
                }
                $seller = M('seller')->where(array('seller_name' => $post['user_name']))->find();
                if ($seller) {
                    $user_where = array(
                        'user_id' => $seller['user_id'],
                    );
                    $user = M('users')->where($user_where)->find();
                    if ($user){
                        if (session('seller_sms_' . $post['phone'])['rand'] == $post['code']) {
                            M('users')->where(array('user_id'=>$user['user_id']))->update([
                                'password' => encrypt($post['password']),
                            ]);
                            session('seller_sms_' . $post['phone'],null);
                            return $this->ajaxReturn(['code' => 1, 'msg' => '密码修改成功']);
                        }else{
                            return $this->ajaxReturn(['code' => 0, 'msg' => '验证码不正确']);
                        }
                    }else{
                        return $this->ajaxReturn(['code' => 0, 'msg' => '账号不存在']);
                    }
                }
                return $this->ajaxReturn(['code' => 0, 'msg' => '店铺不存在']);
                break;
        }
        return $this->fetch();
    }
