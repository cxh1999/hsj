...
class Service extends Base{
	...
	public function refund_info(){
		...
		if(IS_POST){
            ...
            //退款后减去冻结奖金
            $bonusList = M('record_transaction')->where(['ordersn' => $return_goods['order_sn'],'type' => 2,'order_type' => 2])->select();
            if ($bonusList){
                foreach($bonusList as  $value){
                    //修改奖励状态
                    M('record_transaction')->where(['id' => $value['id']])->save(array('bonus_status' => 2));
                    $user = M('users')->where('user_id',$value['user_id'])->find();
                    $user['frozen_funds'] = $user['frozen_funds'] - $value['money'] * (100 - $platform_fee)/100;
                    M('users')->where('user_id',$value['user_id'])->save($user);
                }
            }
	...
	public function paymen_refund($rec_goods){
		if($rec_goods){
			...
			if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'alipay'){
				$return_money = $order['total_amount'];
//				$return_money = ($return_money > $rec_goods['goods_price']) ? $rec_goods['goods_price'] : $return_money;
				if($order['pay_code'] == 'weixin'){
					...
					$data = array('transaction_id'=>$order['transaction_id'],'total_fee'=>(int)($order['order_amount']*100),'refund_fee'=>(int)($return_money*100));
					$result = $payment_obj->payment_refund($data);
					if($result['return_code'] == 'SUCCESS'){
						M('order_goods')->where(array('rec_id'=>$rec_goods['rec_id']))->save(array('status'=>3));
						M('return_goods')->where('order_sn',$order['order_sn'])->save(array('status'=>5));
						$this->success('退款成功');
					}else{
