...
class Store extends Base{
	...
	public function store_list(){
		...
		$Page = new Page($count,10);
        $list = $model->alias('a')->join('store_apply b','a.user_id = b.user_id')->where($map)->order('a.store_id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
		$this->assign('list',$list);
	...
	public function review(){
		...
		if($data['id']){
            ...
            if (empty($data['service_fee'])){
                $this->error('请设置商户流水服务费比例');
            }
            if (empty($data['amount'])){
                $this->error('请设置参与提成金额');
            }
			if($apply && M('store_apply')->where("id=".$data['id'])->save($data)){				
				if($data['apply_state'] == 1){
					...
					$store = array('user_id'=>$apply['user_id'],'seller_name'=>$apply['seller_name'],
							...
							'store_time'=>$time,'store_state'=>0,'store_qq'=>$apply['store_person_qq'],
							'store_end_time'=>$store_end_time,'province_id'=>$apply['company_province'],
							'city_id'=>$apply['company_city'],'district'=>$apply['company_district'],'service_fee'=>$data['service_fee']
					);
    ...
    public function settled_info()
    {
        $id = I('id');
        if($id){
            $info = M('store_settled')->where("id=$id")->find();
            $this->assign('info',$info);
        }
        return $this->fetch();
