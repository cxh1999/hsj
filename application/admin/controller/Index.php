...
class Index extends Base {
     ...
     * 花手绢之宝励数据纠正
     */
    public function jiuzheng(){
        die;
        require_once 'userbaoli.php';
        $arr=arr();
        foreach ($arr as $k=>$v){
            $a=M('users')->where(['user_id'=>$v[0]])->field('name,referrer_id,openid')->find();
            if (empty($a['openid'])){
                M('users')->where(['user_id'=>$v[0]])->update(['referrer_id'=>$v[3]]);
            }
        }
        echo 'OK';
        die;
