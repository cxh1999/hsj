...
class User extends MobileBase
{
    ...
    */
    ...
    {
        //自己点的红包
        $redPacketLog=M('red_packet_log')->where(array('user_id'=>$this->user_id,'type'=>1))->order('start_time desc')->field('user_money,start_time')->select();
        //分享出去之后别点的链接红包
        $redPacketShareMan=M('red_packet_log')->where(array('share_man_id'=>$this->user_id,'type'=>1))->order('start_time desc')->field('user_money,start_time')->select();
        //添加个标识
        ...
        //查出当天时间获取到的红包
        $user_money=M('red_packet_log')->where(array('user_id'=>$this->user_id,'type'=>1))->where('start_time','>=',$timeStare)->where('start_time','<=',$timeEnd)->sum('user_money');
        $share_man_money=M('red_packet_log')->where(array('share_man_id'=>$this->user_id,'type'=>1))->where('start_time','>=',$timeStare)->where('start_time','<=',$timeEnd)->sum('share_man_money');
        //保留两位小数
        $user_money=bcadd($user_money,$share_man_money,2);

        //红包个数
        $redPacketLogCount=M('red_packet_log')->where(array('user_id'=>$this->user_id,'type'=>1))->count();
        $redPacketLogCount+=M('red_packet_log')->where(array('share_man_id'=>$this->user_id,'type'=>1))->count();

        //红包总金额
        $all_user_money=M('red_packet_log')->where(array('user_id'=>$this->user_id,'type'=>1))->sum('user_money');
        $all_user_money+=M('red_packet_log')->where(array('share_man_id'=>$this->user_id,'type'=>1))->sum('user_money');

        //返回数据
        $this->assign('userMoney', empty($user_money)?0:$user_money);
        $this->assign('allUserMoney', empty($all_user_money)?0:$all_user_money);
        $this->assign('lists', $redPacketLog);
        $this->assign('count', empty($redPacketLogCount)?0:$redPacketLogCount);
        return $this->fetch();
    }
...
//        return $this->fetch();
    ...
    {
        //总奖励金额
        $total = M('record_transaction')->where("user_id=$this->user_id AND order_type=2")->sum('money');
        if ($total == ''){
            $total = "0.00";
        }
